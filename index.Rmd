...
## Applied example
This document will help you understand the concepts of the Heckman model in a more graphical way. Here we use the HIV dataset simulated by Marra (2009). The original data set mimics an HIV survey conducted in Zambia in 9 regions, in which 17 different covariates were collected. Here, for ease of explanation, we focus on region #5, and only use 5 predictors: age, marital status (marital), condom use at last intercourse (condom), patient HIV risk (highhiv), and interviewer identity (InterviewerID).

Here we assume that the first 4 predictor variables are included in both the outcome and selection models. In addition, in the selection equation we include InterviewerID as an exclusion constraint variable. Thus, the Heckman model can then be described by the following equations:
