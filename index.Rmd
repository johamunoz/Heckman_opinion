...
Finally, we cluster the results using Rubin's rule for each of the imputed datasets, we use the prevalence.pool function which is also available in the “Additional functions code” tab.
...
From the imputation results, we observe that HIV prevalence from MAR imputation (`r round(res$prevalence[1],2)` %) is close to the prevalence obtained from observable data alone (17.7%). Imputation based on the Heckman imputation model (`r round(res$prevalence[2],2)`%) shows that HIV prevalence could be higher than observed prevalence, suggesting that patients with HIV+ testing are more reluctant to provide HIV consent.
...
Here we simulate a database with the same predictors we used in the previous tab but by setting the $\rho$ value of your preference (please set the value in the slider below)
```{r echo = FALSE}
fluidRow(
...
```
In the figures below we observe that by varying $\rho$ not only does the relationship between the errors of both equations vary (figure on the left), but also the relationship between the latent responses varies (smooth line in the figure on the right).
...
```
The prevalence estimate can best be seen in the table below which shows the total number of HIV+ and HIV- tests according to whether observations were measured or not and the estimated prevalence in each of these groups. The actual prevalence in the total simulated population can also be seen. 
...
On the other hand, when $\rho= 1$, we are in a situation where there is an unobservable variable, such as testing being a requirement for access to an antiretroviral therapy (ART) program, which makes patients more likely to be HIV-positive more willing to consent. Therefore, in this situation the prevalence in the whole population (HIV+ in all patients = 22%) is overestimated based only on observable patients (HIV only in observed patients = 28%).
...
data.sel <- reactive({
  ...
  output$plot_inc_mean <- renderPlot({ 
    datasim<-as.data.table(data.sel())
    summary<-datasim[,.(Meany=mean(y.star,na.rm=T),
                        Meanry=mean(ry.star,na.rm=T)),
                     .(incenc)]
    
    x1<- summary[incenc%in%c("None","Small"), Meanry]
    y1 <- summary[incenc%in%c("None","Small"), Meany]
    x2<- summary[incenc%in%c("Small","Large"), Meanry]
    y2 <- summary[incenc%in%c("Small","Large"), Meany]
    slope1 <- diff(y1)/diff(x1)
    intercept1 <- y1[1]-slope1*x1[1]
    ...
    ggplot(data= as.data.frame(datasim), aes(ry.star,y.star))+
      ...
      ylab(expression('Latent test result ' (Y[i]*'*') %->%''))+
      theme_minimal()+
      geom_point(data=summary,aes(x=Meanry,y=Meany,colour=incenc),size=3)+
      geom_smooth(size=0.5,colour="black",se = F)+
      labs(shape="Measurement",color="Incentive")+
      scale_colour_manual(values=group.colors)+
      ggtitle("Means of incentived patients")+theme_minimal()+
      theme( legend.position = 'bottom')})
...
By using incentives, one can better approximate the curve that relates the latent responses to the selection and outcome responses. Here one can observe how the mean of the groups of people under different types of incentives lie close to the real curve trajectory.

```{r echo = FALSE}
 fluidRow(
    column(12, plotOutput("plot_inc_mean"))
  )
``` 

Here we also shows a table with the number of individuals and their HIV status when tests were observed and unobserved as a function of incentive allocation. The last row of this table shows the actual HIV status of the patients in the survey.
...
``` 
 ...
 In general, it is observed that when $\rho=0$, the MAR imputation method provides the lowest bias estimates. As $\rho$ moves away from zero, the Heckman imputation methods are observed to provide better estimates of HIV prevalence. Most of the time, it is observed that not using ERV in the Heckman model may result in more biased estimates (Not sure about this as the data were simulated by including the Interviewer ID in the selection model's predictor, so No ERV vs ERV from simulated data is a valid comparison, or in this case it is only a problem of model specification?)

The methods with incentives applied here cannot be technically compared with the first three described (MAR, Heckman no ERV, Heckman interviewerID) since the percentage of missing in HIV test is different. That is, in the database with incentives, the test results of individuals whose consent varied due to the incentive are observed. Comparing the results between the incentive methods generally yielded less biased prevalence estimates in complex incentives (ID and interviewer incentive). The prevalence estimate in these incentive methods is sensitive to the proportion of allocated individuals in each of the incentive groups. 
...
prevalence.pool<-function(data, outcome_name) {
  ...
  #Calculate prevalence per imputed dataset
  ...
  return(c(prevalence,ci.lb,ci.ub))
}
